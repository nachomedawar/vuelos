...
const url1 = 'https://www.turismocity.com.ar/vuelos/resultados-a-buenos_aires-BUE?s=MDZ-BUE.07-09-2024&cabinClass=Economy';
const precio1= 25000;
...
console.log(destino);
         ...
         // console.log(elementos.html());
          ...
          elementos.each((index, element) => {
            ...
            var aerolinea= $(element).find('div.tc-col-airline.row span').text().trim();
            var horaIni= $(element).find('div.tc-hhmm').first().text().trim();
            var horaFin= $(element).find('div.tc-hhmm').last().find('em').remove().end().text().trim();
            var precio= $(element).find('em').last().text().trim();

            var ori= $(element).find('span.tc-text-ellipsis.tc-iata').first().text().trim();
            var des= $(element).find('span.tc-text-ellipsis.tc-iata').last().text().trim();
